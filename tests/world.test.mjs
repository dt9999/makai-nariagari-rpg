...
import { WORLD_WIDTH, WORLD_HEIGHT, REGIONS, DISCOVERY_SITES, regionAt, terrainHeight, sitesIn } from '../app/world.ts';
...
test('the original ten territories cover a connected world at least three times the baseline area', () => {
  ...
  assert.equal(REGIONS.reduce((a,r)=>a+r.w*r.h,0),WORLD_WIDTH*WORLD_HEIGHT);
  assert.equal(new Set(REGIONS.map(r=>r.id)).size,10);
  for(let x=0;x<WORLD_WIDTH;x+=250)for(let y=0;y<WORLD_HEIGHT;y+=250){
    const matches=REGIONS.filter(r=>x>=r.x&&x<r.x+r.w&&y>=r.y&&y<r.y+r.h);
...
test('terrain meets across region seams without vertical cliffs caused by mismatched height functions', () => {
  for(let x=6400;x<WORLD_WIDTH;x+=6400)for(let y=50;y<WORLD_HEIGHT;y+=251)
    assert.ok(Math.abs(terrainHeight(x-.01,y)-terrainHeight(x+.01,y))<.01);
  for(let x=50;x<WORLD_WIDTH;x+=251)
    assert.ok(Math.abs(terrainHeight(x,8999.99)-terrainHeight(x,9000.01))<.01);
});
test('destinations have flat foundations for walk-in structures', () => {
  for(const site of DISCOVERY_SITES){
    ...
    for(const [dx,dy] of [[60,0],[-60,0],[0,60],[0,-60]])
      assert.ok(Math.abs(terrainHeight(site.x+dx,site.y+dy)-h)<.0001);
  }
});
